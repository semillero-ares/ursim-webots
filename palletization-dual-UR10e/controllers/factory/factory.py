...
def updateBox():
    ...
    if y > -0.1 and box_id == str(boxCounter):
        boxCounter += 1
        createBox(boxCounter)
    # print(f"[{stem}]\tLast box: {box_id} at y={y:.2f}")
    for box in boxList:
        _, y, _ = box.getField('translation').getSFVec3f()
        if y > 2:
            box.remove()
            boxList.remove(box)
            # print(f"[{stem}]\tRemoved box: {name} at y={y:.2f}")
    pass
...
def setup_rtde_connection():
    ...
    con = [
        rtde.RTDE(HOST, PORT1),
        rtde.RTDE(HOST, PORT2)
    ]
    ...
    while not connected:
        try:
            ...
            simulator.setLabel(1, "Conectado", 0.0,
    ...
    for client in con:
        ...
        inputs = client.send_input_setup(input_names, input_types)
        if not inputs:
    ...
    for client in con:
        if not client.send_output_setup(output_names, output_types, frequency=SAMPLING_FREQUENCY):
...
def get_last_received_state():
    ...
    last_state = None
    state = con[0].receive_buffered()
    while state is not None:
        last_state = state
        # Keep reading until no more states are available
        state = con[0].receive_buffered()
    return last_state
...
while simulator.step(TIME_STEP) != -1:
    ...
    if 'con' not in globals():
        if check_port(HOST, PORT1) and check_port(HOST, PORT2):
